...
def run(args):
    ...
    for model in glob.glob(args.input_pcds+"/*.ply"):
        print("Processing: ", model)
        savePath = jn(args.out_images_path,os.path.basename(model).split(".")[0])
        make_dir(savePath)
        ...
        scene.render.filepath = savePath+"/frame_"  # Output location for the frames
        ...
        for i in tqdm.tqdm(range(args.num_frames), desc="Rendering..."):
            ...
            place_spheres(points,sphere_pool)
            scene.render.filepath = savePath+"/frame_" + str(i)
